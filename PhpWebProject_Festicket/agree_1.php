<?php
$a ='제1조(목적)
이 약관은 FESTICKET(전자상거래 사업자)가 운영하는 웹사이트 FESTICKET(이하 “몰”이라 한다)에서 제공하는 인터넷 관련 서비스(이하 “서비스”라 한다)를 이용함에 있어 사이버 몰과 이용자의 권리?의무 및 책임사항을 규정함을 목적으로 합니다.
...
③"적립금"의 지급기준 및 적립 미율은 회사의 정책상 언제든지 변경할 수 있으며, 변경된 내용은 "몰"을 통해 고지함으로써 그 효력이 발생한다
1.적립금은 회사에서 임의로 지급해드리는 포인트로 현금으로 반환되지 않습니다.
2.적립금은타인에게 양도가 불가하며, 동일한 회원이 여러개의 아이디(ID)를 보유하더라도 합산이 불가합니다.
제14조(환급)
...
?>
<button value="닫기" onclick="window.close()">닫기</button><br>
<script>
</script>
<?=nl2br($a);?>
