...
//저장하고 돌아오면 실행
$sql = "
    INSERT INTO fesitval
        (title, date, date_end, locate, memo, img, created, price)
        VALUES (
            '{$_POST['title']}',
            '{$_POST['date']}',
            '{$_POST['date_end']}',
...
if($result === false){
    echo '<script>alert(\"저장하는 과정에서 문제가 생겼습니다. 관리자에게 문의해 주세요\");</script>';
    error_rog(mysqli_error($conn)); // 아파치 에러로그 라는 곳에 기록되고 사용자에게 보이지 않는다
} else {
    //echo '저장 완료 <a href="sidebar.php">돌아가기</a>';
